...
class TwoPage(Page):
    ...
    left_body = StreamField(
        BaseStreamBlock(), verbose_name="Left body", blank=True
    )
    right_body = StreamField(
        BaseStreamBlock(), verbose_name="Right body", blank=True
    )
    content_panels = Page.content_panels + [
        FieldPanel('introduction', classname="full"),
        ImageChooserPanel('image'),
        StreamFieldPanel('left_body'),
        StreamFieldPanel('right_body'),
    ]
